...
BEGIN
  IF NEW.approved IS DISTINCT FROM OLD.approved THEN
    IF current_setting('app.profile_approval_context', true) = 'duplicate-profile-merge' THEN
      RETURN NEW;
    END IF;
...
BEGIN
  ...
  PERFORM set_config('app.profile_approval_context', 'duplicate-profile-merge', true);

  UPDATE public.profiles
  SET approved = COALESCE(_target_profile.approved, false)
        OR COALESCE(_source_profile.approved, false)
        OR EXISTS (SELECT 1 FROM public.client_program_enrollments WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.group_members WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.client_programs WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.client_tasks WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.measurements WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.client_notes WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.progress_photos WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.food_journal WHERE user_id = _target_user_id)
        OR EXISTS (SELECT 1 FROM public.weekly_check_ins WHERE user_id = _target_user_id)
        OR EXISTS (
          SELECT 1
          FROM jsonb_each_text(public.merge_feature_access_json(_target_profile.feature_access, _source_profile.feature_access))
          WHERE value = 'true'
        ),
...
BEGIN
  ...
  _result := public._merge_duplicate_profiles(_source_user_id, _target_user_id);
  ...
  RETURN _result || jsonb_build_object('approved', true);
END;
$function$;
