...
serve(async (req) => {
  ...
  try {
    ...
    const readResp = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent(sheetTab)}!A:G`,
      {
        headers: {
    ...
    if (!readResp.ok) {
      ...
      console.error("Failed to read sheet:", errText);
    ...
    if (rows.length <= 1) {
      return new Response(JSON.stringify({ success: true, imported: 0, updated: 0, created: 0 }), {
    ...
    for (const row of dataRows) {
      ...
      const [entryId, date, type, category, description, amount, currency] = row;

      // Validate required fields
      if (!date || !type || !category || !description || !amount || !currency) {
      ...
      if (isNaN(numAmount)) {
        console.log("Skipping row with invalid amount:", row);
        continue;
      }

      // Validate type
      if (type !== "expense" && type !== "revenue") {
        console.log("Skipping row with invalid type:", row);
        continue;
      }

      // If entry ID exists and is a valid UUID, try to update
      if (entryId && entryId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
    ...
    return new Response(JSON.stringify({ success: true, imported, updated, created }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
    console.error("import-finance-sheets error:", e);
