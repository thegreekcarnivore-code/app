...
const EMAIL_SUBJECTS_EL: Record<string, string> = {
...
const ROOT_DOMAIN = "thegreekcarnivore.com"
const FROM_DOMAIN = "thegreekcarnivore.com"
...
const SAMPLE_PROJECT_URL = "https://app.thegreekcarnivore.com"
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  console.log('Received auth event', { emailType, email: payload.data.email, run_id, user_metadata: payload.data.user_metadata })
  ...
  // Resolve language from database since webhook payload doesn't include user_metadata
  const recipientEmail = payload.data.email
  let language = payload.data.user_metadata?.language || ''
  ...
  if (!language) {
    try {
      ...
      // Check email_invitations first (covers invite + signup flows)
      ...
      if (invitation?.language) {
        ...
        // For existing users (recovery, magic link, etc.), check auth user metadata
        const { data: { users } } = await adminClient.auth.admin.listUsers({ filter: recipientEmail, perPage: 1 })
        if (users?.[0]?.user_metadata?.language) {
          language = users[0].user_metadata.language
        }
      }
    } catch (err) {
      console.error('Failed to resolve language from DB', { error: err, run_id })
    }
  }

  if (!language) language = 'en'
  console.log('Resolved language', { language, email: recipientEmail, run_id })
  ...
  const templateProps = {
    ...
    language,
    programName: payload.data.user_metadata?.program_name || null,
    startDate: payload.data.user_metadata?.start_date || null,
  }
    ...
    result = await sendLovableEmail(
      {
        ...
        subject: language === 'el'
          ? (EMAIL_SUBJECTS_EL[emailType] || 'Ειδοποιηση')
