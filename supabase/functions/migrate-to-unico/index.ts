...
const UNICO_MONTHLY_CENTS = 4700; // €47/mo locked

const log = (step: string, details?: unknown) => {
  const tail = details ? ` - ${JSON.stringify(details)}` : "";
  console.log(`[MIGRATE-TO-UNICO] ${step}${tail}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    log("Profiles loaded", { count: profiles?.length ?? 0 });
    ...
    for (const profile of profiles ?? []) {
      ...
      try {
        ...
        const latest = invoices.data
          .filter((inv) => inv.amount_paid > UNICO_MONTHLY_CENTS)
        ...
        const overpaidCents = Math.max(0, latest.amount_paid - UNICO_MONTHLY_CENTS);
        ...
        const refund = await stripe.refunds.create({
          ...
          metadata: { migration: "unico_cutover_2026_04_28" },
        });
